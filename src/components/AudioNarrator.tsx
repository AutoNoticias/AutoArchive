  ...
  return (
    <>
      {partsWithIndices.map((p, i) => {
        ...
        const isPast = activePartIndex !== -1 && i < activePartIndex;
        
        return (
          <span
            key={i}
            className={`transition-all duration-150 inline-block rounded px-1 py-0.5 ${
              isCurrentlySpoken
                ? 'bg-amber-400 text-black font-black scale-110 shadow-[0_0_15px_rgba(251,191,36,0.9)] z-10 relative'
                : isPast
...
export const AudioNarrator: React.FC<AudioNarratorProps> = ({
  ...
  // ==========================================
  // 1. SUBTITLES & LIVE TRANSCRIPT MODAL SHEET
  // ==========================================
  const renderSubtitlesModalSheet = () => {
    ...
    return (
      <div
        className="fixed inset-0 z-[1000] bg-[#040810]/95 backdrop-blur-3xl flex flex-col items-center animate-in fade-in duration-300"
        onClick={() => setShowMobileSheet(false)}
      >
        <div
          className="w-full max-w-4xl h-full flex flex-col p-6 md:p-10"
        ...
        >
          ...
          <div className="flex items-center justify-between pb-4 border-b border-white/10 shrink-0">
            ...
            <button
              onClick={() => setShowMobileSheet(false)}
              className="p-2 rounded-full bg-white/5 hover:bg-white/15 text-white/80 hover:text-white transition-all duration-200 active:scale-95 cursor-pointer"
          ...
          <div className="mt-6 shrink-0">
            <span className="text-xs font-mono font-bold tracking-widest text-[#8ea4be] uppercase bg-white/5 px-3 py-1 rounded-full border border-white/10">
              Capítulo {activeChap?.number} • {activeChap?.title}
            </span>
          </div>

          {/* TRANSCRIPT SCROLL CONTAINER (SPOTIFY LYRICS READER STYLE) */}
          <div className="flex-1 overflow-y-auto custom-scrollbar my-8 py-8 flex flex-col gap-8 md:gap-10 pr-2">
            {currentChapterItems.length === 0 ? (
              <p className="text-sm text-white/40 text-center py-10 font-mono">Cargando la transcripción...</p>
            ) : (
              currentChapterItems.map((item, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`text-left transition-all duration-300 outline-none focus:outline-none border-l-4 pl-6 cursor-pointer block w-full py-2 group ${
                      isActive
                        ? 'border-emerald-400 text-white font-extrabold text-xl md:text-3xl drop-shadow-[0_0_15px_rgba(255,255,255,0.45)] scale-[1.01] translate-x-1'
                        : 'border-transparent text-white/20 hover:text-white/50 text-lg md:text-2xl font-bold'
                    }`}
                  >
                    <p className="leading-relaxed md:leading-loose tracking-tight select-none">
  ...
  return (
    <>
      ...
      <aside
        ...
        className="fixed bottom-[78px] md:bottom-6 left-4 right-4 md:left-1/2 md:-translate-x-1/2 md:w-[92%] md:max-w-5xl z-[980] animate-in slide-in-from-bottom-6 duration-300"
      >
        {/* Main Spotify Capsule Layout */}
        <div
          className="bg-slate-950/90 backdrop-blur-2xl border border-white/20 shadow-[0_15px_45px_rgba(0,0,0,0.95)] rounded-full px-4 md:px-8 py-3 flex items-center justify-between gap-3 md:gap-4 relative overflow-hidden"
          style={{ borderLeftWidth: '6px', borderLeftColor: accentColor }}
        >
          {/* 1. Left Column: Album Art & Chapter Info */}
          <div className="flex items-center gap-3 min-w-0 max-w-[150px] xs:max-w-[200px] sm:max-w-[260px] md:max-w-[320px]">
            {/* Spinning Disc / Cover */}
            <div
              className={`w-9 h-9 md:w-10 md:h-10 rounded-full flex items-center justify-center shrink-0 border shadow-md relative overflow-hidden ${
            ...
            >
              {isPlaying ? (
                <div className="flex items-end gap-0.5 h-3">
                  ...
                  <span className="w-0.5 bg-emerald-400 rounded-full animate-bounce [animation-delay:-0.15s] h-3.5" />
            ...
            <div className="min-w-0 flex flex-col">
              <span className="text-[9px] font-mono font-black tracking-widest text-[#8ea4be] uppercase truncate">
                {documentaryTitle}
              </span>
              <span className="text-xs font-black text-white truncate leading-tight mt-0.5 font-sans">
          ...
          <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
            ...
            <button
              ...
              className="p-1.5 sm:p-2.5 rounded-full hover:bg-white/10 disabled:opacity-20 text-white/80 hover:text-white transition-all cursor-pointer"
            ...
            <button
              ...
              className="w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center active:scale-90 transition-transform shadow-lg cursor-pointer shrink-0"
              style={{
          ...
          <div className="flex items-center gap-1.5 md:gap-2.5 shrink-0">
            ...
            <button
              onClick={toggleMusic}
              className={`p-1.5 sm:p-2.5 rounded-full border transition-all cursor-pointer hidden sm:flex ${
                musicEnabled
            ...
            <button
              ...
              className="px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-full bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 font-mono font-bold text-[10px] md:text-xs flex items-center gap-1.5 active:scale-95 transition-all cursor-pointer"
              title="Ver transcripción y subtítulos completos"
            >
